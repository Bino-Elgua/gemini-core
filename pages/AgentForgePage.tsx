                 ...
                 <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                       ...
                       <div className="bg-brand-900/10 border border-brand-500/20 p-10 rounded-[2.5rem] flex flex-col justify-center relative overflow-hidden group shadow-2xl">
                          ...
                          <button onClick={async () => { setIsGeneratingSonic(true); try { const result = await sonicService.generateVoiceProfile(sonicName, 'google'); console.log('Voice generated:', result); } finally { setIsGeneratingSonic(false); } }} disabled={isGeneratingSonic} className="w-fit px-6 py-2.5 bg-brand-600 hover:bg-brand-500 disabled:opacity-50 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-xl transition-all flex items-center gap-2"><Mic className="w-3 h-3" /> {isGeneratingSonic ? 'SYNTHESIZING...' : 'GENERATE VOICE'}</button>
                       </div>
                    </div>
